...
for device in $pci_devices; do
    ...
    mlx_device=$(rdma link show | grep "$eth_device" | awk '{print $2}' | cut -d'/' -f1)
...
mst start
mst status -v
for device in $mlx_devices; do cma_roce_tos -d $device -t 96 ; done
for device in $mlx_devices; do echo 96 > /sys/class/infiniband/$device/tc/1/traffic_class; done
for iface in $altnames; do mlnx_qos -i $iface --pfc=0,0,0,1,0,0,0,0 --trust=dscp; done
for device in $pci_devices; do mlxreg -d $device --set "ipg=0x00000019" --reg_name PIPG --indexes "local_port=1,lp_msb=0" -y ; done
...
firstip=$(echo $altnames[0] | awk '{print $1}')
